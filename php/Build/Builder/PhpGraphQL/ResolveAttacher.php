...
class ResolveAttacher {
        ...
        return Token::multi(
            ...
            $data->getEnvironment()->getBuild()->getInternalPermissionChecks()
                ? Token::multi(
                    ...
                    Token::textnl('$type = (new \\ReflectionClass($object))->getShortName();'),
                    Token::textnl('$method = \'check\' . \\ucfirst($type);'),
    ...
    private function buildTypePageInfo(DataDef $data): Token {
        return Token::multi(
            ...
            Token::textnlpush('return $item->getId() === null'),
            Token::textnl('? null'),
            Token::textnlpop(': $item->getType() . \',\' . $item->getId();'),
            Token::pop(),
            Token::textnl('}'),
